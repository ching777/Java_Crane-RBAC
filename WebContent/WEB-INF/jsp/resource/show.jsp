            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-content">
                        <form class="form-horizontal m-t" id="resourceForm" action="sys/resource/updateNode" method="post">
                            ...
                            <c:if test="${resource.parent_id==-1 }">
                                <div class="form-group">
                                    ...
                                    <select class="form-control m-b" name="parent_id" id="parent_id">
                                        <c:forEach items="${parentNodeList }" var="parent">
                                            <option value="${parent.resource_id }"
                                                <c:if test="${resource.parent_id eq parent.resource_id }">
                                                    selected="selected"
                                                </c:if>
                                            >
                            ...
                            <div class="form-group">
                                <div class="col-sm-4 col-sm-offset-3">
                                    <button class="btn btn-primary" type="button" id="updateBtn" onclick="updateNode()">更新</button>
                                    <!-- [如果你删除的是父节点,那么如果存在子节点不允许删除] -->
                                    <button class="btn btn-primary" type="button" id="deleteBtn" onclick="del()">删除</button>
                                </div>
                            </div>
                            
                            <%-- 隐藏 --%>
                            <input type="hidden" name="resource_id" id="resource_id" value="${resource.resource_id }">
                            <input type="hidden" id="old_resource_name" value="${resource.resource_name}">
                            <input type="hidden" id="old_icon" value="${resource.icon }">
                            <input type="hidden" id="old_url" value="${resource.url}">
